        ...
        public void AgregarTabla(DatosToExcelObject tabla)
        {
            ...
            if (tabla.Datos != null)
            {
                foreach (DataColumn c in tabla.Datos.Columns)
                {
                    ...
                    worksheet.Cell(row, col).Value = texto;
                    if (tabla.markBorder)
                    {
                        worksheet.Cell(row, col).Style.Border.OutsideBorder = XLBorderStyleValues.Thin;
                        worksheet.Cell(row, col).Style.Border.OutsideBorderColor = XLColor.FromHtml(tabla.DefaultBorderColor.ToHex());
                    ...
                    worksheet.Cell(row, col).Style.Fill.PatternType = XLFillPatternValues.Solid;
                    worksheet.Cell(row, col).Style.Fill.BackgroundColor = XLColor.FromHtml(tabla.RGBHeaderBackColor.ToHex());
                    worksheet.Cell(row, col).Style.Font.FontColor = XLColor.FromHtml(tabla.RGBHeaderForeColor.ToHex());
                    worksheet.Cell(row, col).Style.Font.FontSize = float.Parse((tabla.DefaultForeSize + 1).ToString());
                    switch (c.DataType.Name.ToLower())
                    {
                        case "int16":
                        case "int32":
            ...
            if (tabla.Agrupaciones.Count > 0)
            {
                if (row == 1)
                {
                    ...
                    worksheet.Row(row).InsertRowsAbove(1);
                    ini_row++;
                }
                else
                {
                ...
                //mantengo a row en el numero anterior pues necesito escribir en la nueva fila agregada
                //pero la inicial si vale uno mas
            ...
            if (tabla.Datos != null)
            {
                foreach (DataRow fila in tabla.Datos.Rows)
                {
                    row++;
                    foreach (DataColumn c in tabla.Datos.Columns)
                    {
                        ...
                        {
                            worksheet.Cell(row, col).Style.Fill.PatternType = XLFillPatternValues.Solid;
                            worksheet.Cell(row, col).Style.Border.OutsideBorderColor = XLColor.FromHtml(tabla.RGBAlterBackColor.ToHex());

                        }
                        worksheet.Cell(row, col).Style.Font.FontColor = XLColor.FromHtml(tabla.RGBForeColor.ToHex());
                        worksheet.Cell(row, col).Style.Font.FontSize = float.Parse(tabla.DefaultForeSize.ToString());
                        switch (c.DataType.Name.ToLower())
                        {
                            case "int16":
                            ...
                            case "int":
                            case "int64":
                                string valorsdi = fila[c].ToString() ?? "0";
                                if (valorsdi.Trim() == "") valorsdi = "0";
                                int valordin = 0;
                                if (int.TryParse(valorsdi, out valordin))
                                {
                                    worksheet.Cell(row, col).Value = valordin;
                                }
                                else
                                {
                                    worksheet.Cell(row, col).Value = valorsdi;
                                }
                                ...
                                break;
                            // workbook.Worksheets[nombre].Cell(row, col).DataValidation.AddDecimalDataValidation();
                            case "double":
                                string valorsd = fila[c].ToString() ?? "0";
                                if (valorsd.Trim() == "") valorsd = "0.00";
                                double valordd = 0;
                                if (double.TryParse(valorsd, out valordd))
                                {
                                    worksheet.Cell(row, col).Value = valordd;
                                }
                                else
                                {
                                    worksheet.Cell(row, col).Value = valorsd;
                                }
                                ...
                                break;
                            case "decimal":
                                string valor = fila[c].ToString() ?? "0";
                                if (valor.Trim() == "") valor = "0.00";
                                decimal valord = 0;
                                if (decimal.TryParse(valor, out valord))
                                {
                                    worksheet.Cell(row, col).Value = valord;
                                }
                                else
                                {
                                    worksheet.Cell(row, col).Value = valor;
                                ...
                                break;
                            case "datetime":
                                string v = fila[c].ToString() ?? "0";
                                if (v != "") v = DateTime.Parse(v).ToString("dd/MM/yyyy");
                                worksheet.Cell(row, col).Value = v;
                                break;
                            default:
                                worksheet.Cell(row, col).Value = fila[c].ToString();
                                break;
                        }
                ...
                if (tabla.TotalFields.Count > 0)
                {
                    ...
                    foreach (DataColumn c in tabla.Datos.Columns)
                    {
                        decimal valor = 0;
                        bool estotal = false;
                        foreach (string tot in tabla.TotalFields)
                        {
                            string[] totar = tot.Split('|');
                            string nn = tot;
                            decimal cv = 0;
                            if (totar.Length > 1)
                            {
                                cv = decimal.Parse(totar[1], NumberStyles.AllowExponent | NumberStyles.AllowLeadingSign | NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture);
                                nn = totar[0];
                            }
                            if (c.ColumnName.ToLower() == nn.ToLower())
                            {
                                ...
                                else
                                {
                                    for (int i = 0; i < tabla.Datos.Rows.Count; i++)
                                    {
                                        if (tabla.Datos.Rows[i][c].ToString() != "")
                                        {
                                            valor += decimal.Parse(tabla.Datos.Rows[i][c].ToString() ?? "0", NumberStyles.AllowExponent | NumberStyles.AllowLeadingSign | NumberStyles.AllowDecimalPoint, CultureInfo.InvariantCulture);
                                        }
                                    }
                                }
                                break;
                ...
                clon_row.Reset();
                //worksheet.Columns().AdjustToContents();
                col = ini_col + tabla.Datos.Columns.Count;
            }
            ...
            row = ini_row;
            //voy al final de la tabla fila inicial columna externa ( no la ultima columna sino la siguiente)
